...
public class FrmTipoProducto extends AbstractFrm<TipoProducto> implements Serializable {
    ...
    public TipoProducto getObjetoPorId(String id) {
        fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Seleccionado", "Seleccionado"));
        if (id != null && this.modelo != null && this.modelo.getWrappedData() != null) {
            
            return this.modelo.getWrappedData().stream().filter(r -> r.getIdTipoProducto().toString().equals(id))
                    .findFirst()
                    .orElseGet(() -> {
                        Logger.getLogger("no hay objeto");
                        return null;
